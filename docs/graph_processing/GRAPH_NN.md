...
Factorization based approaches will want to use matrix factorization techniques on adjacency or similarity matrices to derive node embeddings - these could be bucketed into [Graph Kernel Methods](/docs/graph_processing/GRAPH_NN.md#graph-kernel-methods) as matrix factorization is not a parameter learning method, but it's mentioned here for the sake of completeness
...
Self-supervised learning on graphs involves training Graph Neural Networks (GNNs) to learn useful node or graph representations without relying on explicit labels - similar to [BERT](/docs/transformer_and_llm/BERT.md) in NLP, self-supervised GNNs create pretext tasks that encourage the model to capture structural and semantic information from the graph itself. There's no direct need for labeled data, as the model learns from the inherent properties of the graph. Some typical tasks and types are:
- **Node Masking**: Randomly mask a subset of node features and train the GNN to predict the masked features based on the surrounding context. This encourages the model to learn meaningful representations of nodes by leveraging their neighbors
- **Edge Prediction**: Train the GNN to predict the existence of edges between pairs of nodes. This can be done by removing some edges from the graph and asking the model to reconstruct them, helping it learn the connectivity patterns in the graph
- **Graph Contrastive Learning**: Create positive and negative pairs of nodes or subgraphs and train the GNN to maximize the similarity of positive pairs while minimizing the similarity of negative pairs. This encourages the model to learn discriminative features that capture the relationships between nodes
- **Graph Auto Encoders**: Use an encoder-decoder architecture where the GNN encodes the graph structure into a latent space, and the decoder reconstructs the original graph from the latent representations. This helps the model learn compact and informative representations of the graph
- **Context Prediction**: Train the GNN to predict the context of a node, such as its neighbors or the subgraph it belongs to. This encourages the model to capture local structural information in the graph

Realistically graphs are still stored in the same way other data has to be stored, so might as well use self-supervised pre-training to learn good node embeddings that can be fine-tuned for downstream tasks like node classification, link prediction, or graph classification 

### Graph Masked Auto Encoders
Graph Masked Auto Encoders (GMAE) are a type of self-supervised learning model designed for graph-structured data - they extend other masked auto encoders in NLP and CV, and are also seen as a more advanced alternative to auto encoders, contrastive tasks, and other self-supervised methods for graphs

The [GraphMAE PDF Paper](https://arxiv.org/abs/2205.10803) outlines a lot of this information, with the main highlights being:
- Contrastive learning in graphs is tricky because data augmentation can easily destroy important structural information
- Contrastive learning on specific tasks is still dominant over typical Graph AE's
- Most tasks are currently focused on node and graph classification, while link prediction is important but has been harder to implement well
- Both contrastive learning and GAE's suffer from training issues and reqiure complex mitigation strategies like negative sampling, hard negative mining, momentum upadtes, and exponential moving averages
    - Negative sampling and augmenation techniques are non-trivial, as they require careful design to avoid information loss and to preserve some sort of signal
    - This lended to the need for "high quality data augmentation" which requires traversal, business logic, etc
        - Cropping a photograph is typically easier than cutting a graph, and there are typical methods like inversion and color jittering that are known to preserve some signal
- Self-supervised auto-encoding can naturally avoid these issues by having an objective to directly reconstruct the original data, without needing complex augmentations or negative samples
    - There are multiple auto encoding methods in graphs focused on specific objectives:
        - VGAE: Variational Graph Auto-Encoders for link prediction and missing edges
        - EP: Proposed to recover vertex features
        - GPT-GNN: Uses an auto-regressive model to predict node features and edge reconstruction iteratively 
        - ARVGA (Adversarially Regularized Variational Graph Auto-Encoder), MGAE (Marginalized Graph Auto-Encoder), GALA (Graph Auto-Encoder with Local and global structure preservation), and AGE (Adversarial Graph Embedding) are other examples of graph auto-encoders that focus on link prediction and graph clustering
    - These self-supervised methods showcase the ability to avoid contrastive issues as it's objective is to directly reconstruct input graph data
    - ***These GAE have not succeeded in achieving a comprehensive outperformance over contractive SSL methods***, but they are much easier to implement
- Therefore, the desire of GraphMAE is to create a simple, effective, and general GAE framework that can work across many tasks without complex augmentations or negative sampling

#### Challenges of Current GAE Architectures
None of the aformentioned GAE methods have been able to comprehensively outperform contrastive SSL methods, and although each of them covers specific topics, none of them have been able to collectively deal with all of the challenges of GAE's:
1. Structure information may be over-emphasized 
    - This mostly means "if a GAE uses link prediction as the objective, it will be good at link prediction and node clustering"
    - It's saying the actual physical closeness of the nodes may be over-emphasized, and other tasks like node classification may suffer
2. Feature reconstruction without corruption may not be robust
    - A number of GAE architectures use "vanilla" feature reconstruction as the objective, which may lead to overfitting and poor generalization
    - Denoising autoencoders and corrupted inputs (with attempt at recovery) are widely adopted in NLP, so they will probably work well in graphs too
3. The mean squared error (MSE) can be sensitive and unstable
    - All existing GAE (in this paper) with feature reconstruction use MSE loss, which can suffer from varied feature vector norms and curse of dimensionality
    - i.e. if one node has a feature vector of all 1's and another has all 0.1's, the MSE loss will be dominated by the first node
4. The decoder architectures are of little expressiveness
    - Feature vectors in graphs are less informative meaning a trivial decoder may not be strong enough to bridge the gaps between encoder representations and decoder targets

The ultimate goal of GMAE's is to address these challenges, and to further enable GAE's to match or outperform contrastive SSL methods across a variety of graph tasks

#### GraphMAE Architecture Choices
GraphMAE's have architecture components similar to any masked auto encoder architectures in NLP / CV, and then there are some critical design choices that help GMAE's specifically 

The architecture of GraphMAE consists of 3 main components:
- **Encoder**: A GNN-based encoder that takes in the corrupted graph (with masked node features) and produces latent node representations
- **Masking Strategy**: A strategy to randomly mask a portion of the node features during training, forcing the model to learn robust representations
- **Decoder**: A lightweight decoder that reconstructs the original node features from the latent representations produced by the encoder

Design choices:
- **Masked feature reconstruction** is used as the pretext task, which encourages the model to learn robust node representations by predicting missing features based on the surrounding context
    - This is different from typical edge reconstruction tasks used in other GAE's, and helps mitigate over-emphasis on structure information
- **Scaled cosine error**, instead of using MSE, which is beneficial when feature vectors vary in magnitudes
    - Most node attributes may be feature vectors or simple numeric numbers - if it's the former then cosine error, the latter can continue to utilize MSE
    - If there are multiple attribute types, hybrid loss functions with mixed weight parameters can be used
    - Scaling the cosine error helps tackle imbalance between easy and hard samples during reconstruction
- **Re-mask decoding** will re-mask the encoders output embedding of masked nodes before they're fed into the decoder
    - Input --> Mask features --> Encoder --> Latent Embeddings --> Re-mask masked nodes --> Decoder --> Reconstructed Features
- **Gnn decoders** are used instead of simple MLP decoders, allowing the decoder to leverage graph structure information during reconstruction
    - This helps bridge the gap between encoder representations and decoder targets, improving reconstruction quality

#### Related Architectures

##### Contrastive Self-Supervised Learning
Contrastive self supervised graph learning methods are still dominant in the field - they aim to encourage alignment between label-invariant distributions, and encourage uniformity across other distributions. Label invariant distributions are those that remain unchanged under certain transformations or augmentations of the graph data, such as node feature masking or edge perturbations. By aligning these distributions, the model learns to capture essential structural and semantic information from the graph that is relevant for downstream tasks. Uniformity across other distributions helps prevent collapse (where all node embeddings become similar) and encourages diversity in the learned representations. It wants to transform input data in a way that helps to preserve similarities, and encourages differences to be evenly distributed away from each other 

- **Negative sampling** is a large part of the **uniformity** portion of contrastive SSL methods, where negative samples (dissimilar pairs) are generated to help the model learn to distinguish between similar and dissimilar nodes or subgraphs
    - This can be done through random sampling, hard negative mining, or other techniques
    - Choosing a "negative" sample is not as easy as it sounds, as in graphs many nodes are related in some way, and so picking truly dissimilar pairs can be challenging
- **Data augmentation** is a large part of the **alignment** portion of contrastive SSL methods, where the goal is to create different views of the same graph data that preserve essential information while introducing some variability
    - Common graph augmentations include node feature masking, edge perturbations (adding/removing edges), subgraph sampling, and attribute masking
    - The challenge is to design augmentations that preserve important structural and semantic information while introducing enough variability to encourage robust representation learning

##### Generative Self Supervised Graph Learning
Generative methods performance on graphs falls behind contrastive methods by a large number, but ultimately they're easier to implement and don't require complex negative sampling or data augmentation strategies

Generative self-supervised graph learning methods focus on reconstructing or generating graph data from learned representations - they aim to capture the underlying structure and semantics of the graph by learning to generate node features, edges, or entire subgraphs. This is typically done through autoencoder architectures, where an encoder learns to map graph data into a latent space, and a decoder reconstructs the original graph from the latent representations

After the autoencoder is trained, you can use the encoder to generate node embeddings for downstream tasks like node classification, link prediction, or graph classification. The learned embeddings capture important structural and semantic information from the graph, which can improve performance on these tasks

Autoregressive models will decompose joint probability distributions as a product of conditionals, allowing for sequential generation of graph components (nodes, edges) based on previously generated elements. The whole "decomposing into joint probabilities" just means that you can generate one part of the graph at a time, conditioning on what you've already generated. This is done via models like RNNs or Transformers adapted for graph data, where the model learns to predict the next node or edge given the current state of the graph

### Graph MAE Approach
>>>>>>> 5067a39 (Push 2 shove):docs/graph_processing/GRAPH_NN.md
