...
\end{pmatrix}
$$
Or maybe we can make it percentage of video watched through (it would be some [numeric feature standardization](/docs/technical%20writing/nn_and_llm/EMBEDDINGS.md#numeric-features)) which would bring each number into the standardized range of $[0, 1]$
$$
M = \begin{pmatrix}
    .9 & .2 & \cdots & .1 \\
    .35 & .5 & \cdots & .25 \\
...
- We can use [Stochastic Gradient Descent (SGD)](/docs/technical%20writing/nn_and_llm/LOSS_FUNCTIONS.md#stochastic-gradient-descent) or [Weighted Alternating Least Squares (WALS)](/docs/technical%20writing/nn_and_llm/LOSS_FUNCTIONS.md#weighted-alternating-least-squares)
...
- [A Softmax Layer](/docs/technical%20writing/nn_and_llm/LOSS_FUNCTIONS.md#softmax) will allow us to do multi-class classification, where we basically predict the probability of usage over an entire corpus / corpus subset of videos
    ...
    - Since our output is a probability distribution that's comparable to truth (all 0's and a 1) we can use [cross-entropy loss function](/docs/technical%20writing/nn_and_llm/LOSS_FUNCTIONS.md#cross-entropy)
