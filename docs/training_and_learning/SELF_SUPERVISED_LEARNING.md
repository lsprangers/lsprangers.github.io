...
### Reconstruction
How can you create models that can re-create photos?

#### Image Coloring
If you have a set of 1 million images, how can you create latent representations of photo colors and images?

you can pair up (original phot, greyscale) pairs, after that you can train a model to "color in" the photos and use [L2 Loss](/docs/training_and_learning/LOSS_FUNCTIONS.md#l2-loss) between predicted "colored in" photo and the original photo 
...
Similar idea to above Coloring, except you prepare training pairs of (small, upscaled) images by downsampling existing images

#### Image Inpaiting
This is one of the best self supervised methods for learning latent representations in pictures

In this architecture you randomly remove some section of the picture, and the goal is to re-insert the missing section

In this ![image inpainting example](./images/image_inpainting_example.png) you see they pull out random sections of the photo, and then we'll end up trying to use a [GAN Architecture](/docs/training_and_learning/GAN.md#gan) to regenerate the missing portion
...
The example used in blog(s) is typically pattern matching - when there is one image on the left side, and 4 possible images to choose from on the right side, and you have to choose the "closest" image from the right to pair with the left

you have to pick from a set of objects to *contrast* to another object
...
Given you want to use [Contrastive Learning](#contrastive-learning), how can you use this type of learning alongside self supervised learning, to formulate an architecture where you can use self supervision without labeling?
...
***The task*** is to maximize the similarity between $Similarity(x_i, x_j)$ - once this is done you can reuse the output of the encoders, $h_i$ in downstream tasks. This effectively allows us to create useful encoder(s) for representing characteristics of images
