...
you can continue down the chain and get to a callstack, repeatedly calling `node.left`, of `[A, B, D, F]` and once `F` returns our callstack is `[A, B, D]`
```
    A
B       C
D
...
class Stack:
    ...
    def push(self, val):
        ...
        # always use self.tail.prev b/c you want to push to empty stack
        prev = self.tail.prev 
        prev.next = new
        new.prev = prev
        new.next = self.tail        
