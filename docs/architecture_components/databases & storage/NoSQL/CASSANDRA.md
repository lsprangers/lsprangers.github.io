...
To be fair, it can be used and abused in many ways, but at the end of the day we're partitioning data across nodes via a `partitionKey`, and accessing our data via that `partitionKey`, so it can be equated to KV, but you could also get multiple keys and some people abuse this in weird ways
...
- ***RowKey***: Given this "set" (Row) of Attributes that we've stored in this "Map", we'd most likely want to access this entire Row in one go
    - AKA Primary Key
    - you could make a Map of Maps
        - In practice, you mostly store a Map of K:V objects, or a Map pointing to a List
...
- Not all Rows have the same Attributes
    - you might have more information for John Smith than you do Jane Doe
    - Because of this, for the actual underlying storage mechanisms you may need to decouple our Rows and not have such rigid schemas
...
- ***Tables***:
    ...
    - A dummy example would be: `create table person ( person_id int primary key, fname text, lname text, dateofbirth timestamp, email text, phone text );`
        - you use `person_id` as the Primary Key or Row Key
        - `insert into person (person_id,fname, lname, dateofbirth, email, phone) values ( 123, 'Chris', 'Williams', '1960-05-20', 'chris.williams@test.com', '099045769');`
            - you insert a Row for `Chris Williams` with `person_id = 123`
    ...
    - you could then insert new Rows with different information / Columns
...
- ***Partition Key***: 
    ...
    - If there are multiple columns in a Partition Key, you need to include those in every query
        - `PRIMARY_KEY((name, email), birthdate)` would route data to Partitions based on `(name, email)` so you would need to include those 2 pieces of information everytime you want to lookup the data
- ***Partitioner***
    - The Partitioner is a program / function that will take the ***Partition Key*** and figure out which Node the Row is assigned to
    - Allows us to PUT the Row to the right Node, and to figure out which Node you need to GET from
        - ![Partitioner Circle](./images/cassandra_partitioner.png)
    - you discussed this part in depth in [Partitioning Document of our Distributed KV Store Implementation](/docs/architecture_components/typical_reusable_resources/typical_distributed_kv_store/PARTITION.md)
...
Because of this setup you can say that ***Primary Indexes are Global*** - i.e. the Index itself is spread across multiple Nodes, and realistically it happens because of the *Partitioner* routing PUT and GET to the right Nodes and then using the Hash Table lookup to find the right `PrimaryKey -> Row` to update

### Secondary Index
Building a Secondary Index over `user_email` would allow us to complete that inverse query, but it would basically mean you need to store every single `user_email -> [user_id]` mapping...you see how this could go poorly

User emails most likely will be 1:1 distinct with `user_id`, and so if you have 1Million rows, our Secondary Index would also be another 1Million rows

#### Storage Structure of Secondary Index
- In the above section on [Primary Indexes](#primary-indexes) you mentioned that *Primary Indexes were Global*, and the counter to that is that *Secondary Indexes are Local*
...
- If you have a bunch of `user_id, user_email, state` Columnn, and let's say 1Million rows:
    - Creating a Secondary Index off of `user_email` basically means you replicate a `user_email -> user_id` mapping for every row on every Partition
    - Creating a Secondary Index off of `state` would mean you replicate a `state -> [user_id]` mapping for every row on every Partition, but it would be a lot "cheaper" since we'd only be storing a few `user_id`'s for each state
        - Granted, in both setups, you still do need to store 1Million `user_id` in each Secondary Index
- Query Profile:
...
- ***Integrity***:
    - In the above solution you now have 2 different areas you would need to update any attribute like `user_city`
        - If the update on `user_id` succeeds, and the update on `user_email` fails, you now have inconsistent data
        - ![Per Reference Table](./images/cassandra_per_reference.png)
    - In doing the Per Reference portion, you essentially add in a new Row to another table with False set for each of `user_id` and `user_email`
        ...
        - Therefore, for each denormalized table, on an attribute update, you must include the Reference creation, and update, in each transaction for an Attribute
