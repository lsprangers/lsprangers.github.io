...
There are some other things you can do in terms of indexing, sharding, consistency, and replication that causes differences between different systems

### Implementations
In this scenario you chose to create a distributed, sharded KV store with node ranges based on consistent hashing, and replication done via RAFT
...
Code for this is easiest to just view my crappy [RAFT Repo](/docs/projects/raft/index.md) instead of me trying to recreate it heres
...
#### Scaling
If our database grows too large and is crashing you have 2 options - Horizontal or Vertical Scaling

Vertical scaling equates to "make the compute larger" which reaches limitations very quickly, but for some teams and products this is completely fine and then you can ignore all of the issues that come up with distributed systems!

Horizontal scaling is usually the route taken where you take our database and split it up into subsets

### Durability
Most of the time Durability is covered by [Tree Structures on Disk](./TREE_STRUCTURES_ON_DISK.md) so that you can still retrieve our data in $O(\log n)$ from disk. This helps with durability and allowing us to put more of the overall KV Store onto a single node versus scaling out and trying to fit everything into RAM


#### Sharding
[Sharding](/docs/architecture_components/typical_reusable_resources/typical_distributed_kv_store/SHARDING.md#sharding), which is covered in a sub-document with routing, is useful when you want to have different nodes accept reads and writes for different subsets of our database, and sometimes it's required when our data can't sit on a single machine
...
Sharding can be done horizontally or vertically as well
    - Horizontal sharding is when you split up by rows, so maybe you have `1 a b c` in one node, and `2 d e f` in another
    - Vertical sharding is when you split things up by columns so maybe `a d g` is on one node, and `c f i` in another

#### Replication
For each node in a shard (or the single node in a non-sharded database) you can use [Replication](REPLICATION.md) to solve 2 major problems - Fault Tolerance and Scaling 
...
- Peer-to-peer approach is the approved method in text, which seems to be some form of quorum with replication of data on other nodes as well
    - Nodes are assigned keys based on the partition in the ring, and then there’s a replication factor n that describes how many other physically separate nodes to replicate data on 
        - you can replicate data on next 3 physically different nodes on the ring, so that if our node fails the next nodes already have the data and don’t need to shuffle
    ...
    - Sync vs Async
        ...
        - you can have each one of the partitions have a leader, and multiple replica followers (or some other strategy), where these get updated by the leader and then can become leader if it’s downed
- Basically, you need to decide on a plan based on the requirements of the service, if it’s a high velocity write with high consistency and “always write” needs, then quorum may be the best service
- If it’s low velocity writes, high throughput of reads, and lower consistency then you can do single leader + primary secondary + followers
...
#### Scaling
- you can use partitioning based on hash of initial key
...
- Can use circular partitioning (i.e. modulo based) to ensure equal distribution among nodes
    - ***WRONG*** - you can see, that if you added a new server 5, then all of the modulo calculations would get screwed up and we’d have to shuffle data around
...
- We’ll have to use something like pre-defined partitions or ranges, and rebalance somehow
    - This is a functional requirement that you must be able to scale, so you cannot use pre-defined partition sizes
    - If there is a more consistent data model where you know the data will have some sort of setup, you can pre-define the partitions, but otherwise you can do circular routing and re-size the partitions
...
- Circular Hashing is our best bet here 
    ...
    - you cannot use modulo based hashing, because there would be an entire shuffle for all data, and ultimately it’s never really a good idea to do this unless you’re sure of uniform load, and very infrequent scaling
    - you can use circular hashing which allows minimal shuffle of data between new nodes, and allows us to infinitely scale our nodes and ring sizes

#### Replication
- All depends on read/write throughput and consistency
- *Single leader + multiple followers* - ***WRONG*** - if you have tame velocity for reading and writing
    - Leader takes all writes, and sends data out to replicas
    - Primary is single point of failure
        - *you cannot always write here,* ***you can always read***
...
- *Multiple leader + followers*
    - This means multiple nodes can handle writes, but then you need to figure out the plan for conflicting writes
...
- ***P2P + Quorum***
    ...
    - you set $w + r \gt n$ so that you ensure our reads are consistent
...
- If you have a quorum based approach, and one of the nodes is temporarily down, you can use hinted handoffs and sloppy quorum to combat this
- During a distributed transaction if one of the nodes is temporarily down, you cannot use it in the quorum vote and the availability of the service degrades
    - Sloppy quorum allows us to keep a preference list for all nodes so that the first n healthy nodes from the list can handle operations, and if one node is down you can continue down the list
...
- you need to speed up detection of these inconsistencies
...
- If they’re the same, no reason for us to check anything else
    ...
    - Due to CAP theorem you must choose, and you choose high availability, this comes from always write requirement, and so even if network between nodes fails you still allow writes
- How can you ensure eventual consistency then?
...
- you can force our put API requests to also require some sort of metadata context, and then our get requests also either use or return this data for consumer to reuse if multiple rows returned
...
- Example below for Quorum Read and Writes
    ...
    - Consider r & w
        ...
        - W is the minimum number of nodes for a successful write operation
            - Then if r = 2, it means our system reads from 2 nodes when you have data stored in 3 nodes
        - you must pick r and w so that at least one node is common between them
        - This ensures if there’s inconsistent data, that you will catch it
        - That’s why r + w > n
