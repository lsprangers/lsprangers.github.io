...
you can also "snapshot" that disk at some point in time and copy that over to another physical node
...
- Leader is always a single point of failure
        ...
        - If you have many reads, and you increase # of replicas, then primary write to replicas can become a bottleneck during writes
...
- Instead of storing statements in log, you store transactions, which can ensure consistency with non-deterministic functions
#### Logical (row-based)
...
- Changes made to individual rows are recorded and then replicated to follower nodes
    ...
    - Multi-Leader Replication
        - If the leader fails, then you lose data
        - you could have multiple leaders
...
In the Data Engineering world you use WAL's (Write Ahead Logs) and CDC (Change Data Capture) a lot 
...
WALs solve multiple problems themselves!
    - Fault tolerance since they are written on disk, and after a crash you can replay these logs
    - Replication by having other replicas read this WAL to see what they need to do
    - Logging! you sometimes want to know when transactions / writes happened
...
Each replica has a replication slot, which is basically just the metadata of the last offset it's read
    ...
    - ***you can see here that the two replica's aren't consistent! A request to either would result in 2 different results***
![WAL](images/wal.png)
...
- Quorum is how you handle the write inconsistencies across nodes
- N nodes, W for write success, R for read
- We’ll get the updated value from reading any node as long as w + r > n since at least one of the nodes must have an updated write from which you can read
    - This also means w > n – r which is saying as long as you require having w nodes with the updated consistent value, and there are more write success threshold than total nodes less reads, then you can ensure you get the updated value
- Altogether it’s a schema to ensure that if you read a value, you know it must be updated, because you set such a high threshold it’d be impossible to have quorum otherwise
...
After enough nodes (usually > 1/2) reply they've gotten it (a consensus) you consider the value replicated
    - This reply from a consensus might take a long time, and that would mean a performance and availability reduction 
        - you can still serve other reads and writes from other clients, but that specific update would not be reflected for a while and that would be annoying as a client
...
you would say there's a quorum if more than a certain factor, usually `# nodes / 2`, agree on a value:
- `w` is the write factor.
- `n` is the number of nodes.
- `r` is the read factor.
- As long as `w + r > n`, you are guaranteed to have a consistent majority with replicated data.
...
- Write factor `w = 3`
- Read factor `r = 2`
- you need to pick `w` and `r` such that there's at least one node in common between them, i.e., `w + r > n`.
- `3 + 2 > 4`
- On each write, you know you must update at least 3 nodes in total, and when any node is requested to serve a client, it needs to check with at least 2 other nodes.
...
- Other implementations like CochroachDB allow any node to accept reads and writes and then they use the gossip protocol to send out messages 
    ...
    - When a new read request comes in you need to send read to all nodes, and if majority agree on a value then you can consider it the known current value
