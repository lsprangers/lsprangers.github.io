...
- There are special tokens throughout which help us to have dynamic input representations and language modeling tasks
    - `[CLS]` represents a ***special classification token*** to represent start of sentences, our final end sentences, and tokens you wish to predict
...
- For MLM tasks you basically need to set things up so that any word can't "see itself" and is able to use the contextual positions of all other words
...
- ***Catch***: The `[MASK]` token doesn't appear during downstream feature-based or fine-tuning tasks...
    ...
    - Finally, you can use $T_i$ to predict our actual output token, and that will be compared with $w_i$ using cross entropy loss
...
- 80% of the time:
  ...
  - Loss: Cross Entropy
    - you get the output distribution of the final hidden layer and compare it to the word you `[MASK]`-ed
- 10% of the time:
...
- ***Training Objective:*** to predict, in a binary fashion, the next sentence from a possible next sentence that's presented to us
    - Basically you are going to give a random, possible true, next sentence B for any sentence A and predict 0 or 1 if you think it's the next sentence
- Why do you do this? Tasks like question answering and natural language inference all depend on modeling the *relationship between 2 sentences*
...
- There are generally 2 strategies for applying a pre-trained BERT base model to downstream tasks
    - ***Feature Based***
        ...
        - It boils down to creating and optimizing a new final classification layer
            ...
            - This final classification layer is the entire basis of our downstream task, and uses the pre-trained BERT model for modeling the text representation before you finally use it
        - you could train this at base model training time, but this would involve lots of compute spend each time, or you can ***update the base models weights while training the classification layer*** 
    - ***Fine Tuning***
...
- you can model single sentence, or sentence pairs, in our fine-tuning step, because BERT's self-attention mechanism allows for context to come through at each step
    ...
    - This basically means that if you concatenate two sentences together you can allow the context from sentence B to flow into sentence A because you just treat this as one single sentence, and the semgent (sentence) embedding helps us to distinguish and attend to our sentence pairings.
- For each task you simply input the input-output pairing, and use specific loss functions to ***tweak the parameters end-to-end***
...
- Using a Cloud TPU or GPU you can reproduce the Fine Tuning results in an hour or a few hours respectively
...
- General Language Understanding Evaluation (GLUE)
    ...
    - Ultimately this allows us to label sentences into $K$ distinct labels, where *our new classification layer is the only thing you added!*
...
- Stanford Question Answering Dataset (SQuAD)
    ...
    - *Input:*
        ...
        - you need to find $[i, j]$ from our passage that answers the question
    - *Parameters:* 
        - Pre-trained parameters in $H$
        - Start and End embeddings to help us figure out when to begin and end our answer
            ...
            - Probability of a word $w_i$ being the start of an answer span is $T_i \dot Y$ meaning you find the closest Token to our start embedding, and since our start embedding is learned throughout the process you basically just hope our ***start embedding, after being attended to for a specific question, represents the best starting word***
                - you also wrap the dot product in a softmax, and only calculate this over the words inside of the passage, not our entire vocab
            - Same thing for end
