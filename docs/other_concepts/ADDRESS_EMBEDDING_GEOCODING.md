...
- Basic standardization completed to clean up the text is useful before sending to LibPostal - most of the time you need to do things like:
    - ***Transliteration*** to bring our data from any alphabet type to Latin
        - ***I get that sometimes other languages are drastically more useful, if we're doing address encoding in China you certainly shouldn't convert to Latin alphabet
...
### Similarities
Once we've parsed and normalized our data you can upload it to a service to get compared to other locations you know of
...
`from_node` is typically the starting address of a street, and `to_node` is typically the end. If our `Main Street` has street numbers from $[100, 400]$ and you lookup $200$, then it will go $1/3$ of the way "down the street", known as ***interpolating*** from our starting node to get that lat/long

### Bounding Box Search Space
you can reduce the search space for something like `100 Main St` if you have User context around what the user is looking for. For example if they're in Iowa instead of Illinois, you can filter down to just Iowa addresses and check there...but this can be tricky because cities like Kansas City are on the border of 2 states, so you can't use hierarchies based on lines drawn, but we'd have to use ***bounding boxes***

### Map Zoom
you can use the User context of the location of the world a user is checking on the UI to reduce the search space (if they're looking at a map of USA they probably don't want EU results)

### GeoIP
you can use User context like their GeoIP to further reduce the search space