---
title: longestSubstringWithoutRepeats
...
class Solution:
    ...
    def lengthOfLongestSubstring(self, s):
        ...
        for i in range(len(s)):
            ...
            #   is at an earlier position you would re-configure where start is
            #       which basically means starting over using a new substring
            #       because the maxLength would be updated in else 
            # Also you need to check that start is less than entry of used char because if start
            ...
            if s[i] in usedChar and start <= usedChar[s[i]]:
                ...
                # If you haven't seen this character before then you check if our
                #   old maxLength, or the substring you now have, is larger
                #       since i - start + 1 = length of currently un-repeated substring
                maxLength = max(maxLength, i - start + 1)

            # Lastly you update the usedChar dictionaries input of that char
            usedChar[s[i]] = i
